...
看到 ModularRSI 在终端任务上提了近 5 个点，希望和贵司一起把这套方法扩展到交易和预测环境。想了解贵司目前的数据缺口：哪些领域、什么形式（SFT、偏好数据、评分标准、RL 任务、评测集），以及需要的量级和交付周期。我们可以先做小批量付费试点，质量高，价格有竞争力。
...
Bryan Dai 您好，我是 SimReal 联合创始人[姓名]，[介绍人]推荐我联系您。我们为大模型后训练做 RL 环境、验证器和专家数据，Jane Street、Citadel 的从业者在支持我们，每个环境上线前都做对抗测试。想了解一下贵司现在数据上缺哪些（领域、形式、量级、交付周期），我们可以先做小批量付费试点，质量高，价格有竞争力。您看下周方便约 20 分钟吗？
